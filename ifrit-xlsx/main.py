...
########################################################################
def export_to_xlsx(ennemy_list, game_data):
    ...
    for key, ennemy in ennemy_list.items():
        ...
        if file_index == 127 or file_index > 143:  # Ignoring garbage files
            continue
        file_name = str(file_index) + " - " + ennemy.info_stat_data['monster_name']
        if file_name == '':
            file_name = "Empty"
        ...
        worksheet.write_row(1, COL_ABILITIES, ["Type", "Ability", "Animation", "Type", "Ability", "Animation", "Type", "Ability", "Animation"],
                            cell_format=column_title_style)
        worksheet.write(ROW_DROP_CARD, COL_DROP_CARD, game_data.CARD_DATA['pretty_name'],column_title_style)
        worksheet.write(ROW_DEVOUR, COL_DEVOUR, game_data.DEVOUR_DATA['pretty_name'],column_title_style)
        ...
        worksheet.write(ROW_MONSTER_NAME, COL_MONSTER_INFO + 1, ennemy.info_stat_data['monster_name'], border_style)
        worksheet.write(ROW_MONSTER_NB_ANIMATION, COL_MONSTER_INFO, "Nb animation", row_title_style)
        worksheet.write(ROW_MONSTER_NB_ANIMATION, COL_MONSTER_INFO + 1, ennemy.model_animation_data['nb_animation'], border_style)
        ...
        for i in range(len(ennemy.battle_script_data['battle_text'])):
            ...
            worksheet.write(ROW_MONSTER_COMBAT_TEXT + i, COL_MONSTER_INFO + 1, "{}".format(ennemy.battle_script_data['battle_text'][i]), border_style)
        ...
        for param_name, value in ennemy.info_stat_data.items():
            ...
            try:
                ...
                if param_name in game_data.stat_values:
                    ...
                    worksheet.write(row_index['stat'], COL_STAT, pretty_name, row_title_style)
                    column_index['stat'] += 1
                    ...
                    worksheet.write(ROW_GRAPH_PER_LVL, column_index['graph_stat'], pretty_name, column_title_style)
                    if param_name == 'hp':
                        # Impact 1
                ...
                # Def menu
                    ...
                    for el2 in value:
                        if param_name == 'elem_def':
                            worksheet.write(row_index['def'], column_index['def'], game_data.magic_type_values[index[param_name]], title_magic_style)
                            worksheet.write(row_index['def'], column_index['def'] + 1, el2, magic_style)
                        elif param_name == 'status_def':
                            worksheet.write(row_index['def'], column_index['def'], game_data.status_values[index[param_name]], title_status_style)
                            worksheet.write(row_index['def'], column_index['def'] + 1, el2, status_style)
                        index[param_name] += 1
                        row_index['def'] += 1
                # Item menu (containing draw too)
                elif param_name in ['low_lvl_mag', 'med_lvl_mag', 'high_lvl_mag', 'low_lvl_mug', 'med_lvl_mug', 'high_lvl_mug', 'low_lvl_drop', 'med_lvl_drop',
                                    'high_lvl_drop']:  # Items
                ...
                # Misc menu
                    ...
                    worksheet.write(row_index['misc'], column_index['misc'], pretty_name, row_title_style)
                    if param_name == "mug_rate" or param_name == "drop_rate":  # Percent style need to divide by 100
                        worksheet.write(row_index['misc'], column_index['misc'] + 1, value / 100, percent_style)
                    else:
                        worksheet.write(row_index['misc'], column_index['misc'] + 1, floor(value), border_style)
                    row_index['misc'] += 1
                # Abilities menu
                    ...
                    worksheet.write(ROW_DROP_CARD +1, COL_DROP_CARD, 'Drop', row_title_style)
                    worksheet.write(ROW_DROP_CARD +1, COL_DROP_CARD+1, game_data.card_values[value[0]], border_style)
                    worksheet.write(ROW_DROP_CARD +2, COL_DROP_CARD, 'Mod', row_title_style)
                    worksheet.write(ROW_DROP_CARD +2, COL_DROP_CARD+1, game_data.card_values[value[1]], border_style)
                    worksheet.write(ROW_DROP_CARD +3, COL_DROP_CARD, 'Rare mod', row_title_style)
                    worksheet.write(ROW_DROP_CARD +3, COL_DROP_CARD+1, game_data.card_values[value[2]], border_style)
                elif param_name in ['devour']:
                    worksheet.write(ROW_DEVOUR +1, COL_DEVOUR, 'Low', row_title_style)
                    worksheet.write(ROW_DEVOUR +1, COL_DEVOUR+1, game_data.devour_values[value[0]], border_style)
                    worksheet.write(ROW_DEVOUR +2, COL_DEVOUR, 'Medium', row_title_style)
                    worksheet.write(ROW_DEVOUR +2, COL_DEVOUR+1, game_data.devour_values[value[1]], border_style)
                    worksheet.write(ROW_DEVOUR +3, COL_DEVOUR, 'High', row_title_style)
                    worksheet.write(ROW_DEVOUR +3, COL_DEVOUR+1, game_data.devour_values[value[2]], border_style)
            except IndexError:
                raise IndexError("Unknown error on file {} for monster name {}".format(key, ennemy.info_stat_data['monster_name']))
...
########################################################################
def import_from_xlsx(xlsx_file, ennemy, game_data, output_path):
    """
    ...
    for sheet in wb:
        ...
        for i in range(ROW_DROP_CARD + 1+1, ROW_DROP_CARD + 1+1 + len(current_ennemy.info_stat_data['card'])):
            card_list.append(game_data.card_values.index(sheet.cell(i , COL_DROP_CARD+1+1).value))
        current_ennemy.info_stat_data['card'] = card_list
        # Devour reading
        devour_list = []
        for i in range(ROW_DEVOUR + 1+1, ROW_DEVOUR + 1+1 + len(current_ennemy.info_stat_data['card'])):
            devour_list.append(game_data.devour_values.index(sheet.cell(i , COL_DEVOUR+1+1).value))
...
def xlsx_to_dat(xlsx_file, output_path):
    ennemy = {}
    ...
    import_from_xlsx(xlsx_file, ennemy, game_data, output_path)

    print("Writing data to dat files")
    write_to_dat(ennemy, game_data, output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("xlsx",
                        ...
                        choices=("fs_to_xlsx", "xlsx_to_fs", "both"), default='fs_to_xlsx', nargs='?')
    ...
    parser.add_argument("--lang", help="Choose the language you use", choices=("eng", "fre"), default='eng', nargs='?')
    args = parser.parse_args()

    FILE_BATTLE_SPECIAL_PATH_FORMAT = os.path.join(args.lang, "battle")
    ...
    if args.xlsx == "xlsx_to_fs" or args.xlsx == "both":
        ...
        xlsx_to_dat(FILE_XLSX, FILE_MONSTER_OUTPUT_PATH)
